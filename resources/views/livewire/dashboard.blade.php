                                ...
                                <div class="flex items-center justify-center gap-1.5">
                                    @if(auth()->user()->hasRole(['admin', 'tecnico']))
                                        <select 
                                            ...
                                            class="bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-200 text-xs font-semibold px-2 py-1 rounded-lg border border-slate-200 dark:border-slate-700 focus:outline-none cursor-pointer">
                                        ...
                                        <button 
                                            ...
                                            class="p-1.5 text-indigo-500 hover:bg-indigo-500/10 rounded-lg transition"
                                            title="Bitácora y Fotos">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z"></path></svg>
                                        </button>
                                    @endif

                                    <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $order->client->phone) }}?text={{ urlencode('Hola ' . $order->client->full_name . ', puedes hacer el seguimiento en vivo de tu equipo ' . $order->brand_model . ' ingresando a este enlace: ' . url('/seguimiento/' . $order->uuid)) }}"
                                       target="_blank" 
                                       class="p-1.5 text-emerald-500 hover:bg-emerald-500/10 rounded-lg transition"
                                       title="WhatsApp">
